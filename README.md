...
`src\embedded_pdf.rc` 将 `assets` 目录中的 `郭登宇简历.pdf` 作为标准 Windows `RCDATA` 资源编译进 EXE。程序运行时会：
...
默认输出文件名为 `郭登宇简历.pdf<78 个指定 Unicode 空白占位符>.exe`。如果替换了 PDF，请保留 `assets` 目录中的文件名 `郭登宇简历.pdf`，然后重新运行构建脚本。

源代码和构建文件位于 `src` 目录。构建时会根据当前 Windows 的 `.pdf` 文件关联生成 `src\pdf_default.ico`，并把该图标编译为 EXE 的程序图标。

`assets\郭登宇简历.pdf` 只在编译时使用。最终 EXE 已经包含完整 PDF 资源，运行时不依赖 `assets` 目录或其中的 PDF 文件。
